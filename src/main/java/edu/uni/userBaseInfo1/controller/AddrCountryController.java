...
public class AddrCountryController {
    ...
    public void receive(@PathVariable Long id, HttpServletResponse response) throws IOException {
        ...
        //拼接缓存键名（字符串）
        String cacheName = AddrCountryController.CacheNameHelper.Receive_CacheNamePrefix + id;
        //尝试在缓存中通过键名获取相应的键值
    ...
    public void selectAll(HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = AddrCountryController.CacheNameHelper.ListAll_CacheName;
        String json = cache.get(cacheName);
        if(json == null){
    ...
    public Result create(@RequestBody(required = false) AddrCountry addrCountry){
        ...
        if(addrCountry != null){
            ...
            if(success){
                // 清空相关缓存
                cache.delete(AddrCountryController.CacheNameHelper.ListAll_CacheName);
    ...
    public Result update(@RequestBody(required = false) AddrCountry addrCountry){
        if(addrCountry != null && addrCountry.getId() != null){
            ...
            if(success){
                //清除相应的缓存
                cache.delete(AddrCountryController.CacheNameHelper.Receive_CacheNamePrefix + addrCountry.getId());
                cache.delete(AddrCountryController.CacheNameHelper.ListAll_CacheName);
                return Result.build(ResultType.Success);
            }else{
                return Result.build(ResultType.Failed);
