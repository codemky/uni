...
import edu.uni.userBaseInfo1.service.MyFirstLevelDisciplineService;
...
@Controller
public class MyFirstLevelDisciplineController {

    //把FirstLevelDiscipline的Service层接口所有的方法自动装配到该对象中
    @Autowired
    MyFirstLevelDisciplineService myFirstLevelDisciplineService;
    ...
    @ApiImplicitParam(name = "id", value = "firstLevelDiscipline表的一个id", required = false, dataType = "Long" , paramType = "path")
    @ResponseBody
    public void receive(@PathVariable Long id, HttpServletResponse response) throws IOException {
        //设置返回的数据格式
        response.setContentType("application/json;charset=utf-8");
        //拼接缓存键名（字符串）
        String cacheName = MyFirstLevelDisciplineController.CacheNameHelper.Receive_CacheNamePrefix + id;
        //尝试在缓存中通过键名获取相应的键值
        //因为在Redis中，数据是以”“” "键-值"对 的形式储存的
        String json = cache.get(cacheName);
        //如果在缓存中找不到，那就从数据库里找
        if(json == null){
            FirstLevelDiscipline firstLevelDiscipline = myFirstLevelDisciplineService.selectFirstLevelDisciplineById(id);
    ...
    public void selectAll(HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = MyFirstLevelDisciplineController.CacheNameHelper.ListAll_CacheName;
        String json = cache.get(cacheName);
        if(json == null){
            json = Result.build(ResultType.Success)
                    .appendData("firstLevelDisciplines", myFirstLevelDisciplineService.selectAllFirstLevelDisciplines()).convertIntoJSON();
            cache.set(cacheName,json);
        }
        response.getWriter().write(json);
    }
    ...
    public Result create(@RequestBody(required = false) FirstLevelDiscipline firstLevelDiscipline){
        //检验页面传来的对象是否存在
        if(firstLevelDiscipline != null){
            boolean success = myFirstLevelDisciplineService.insertFirstLevelDiscipline(firstLevelDiscipline);
            if(success){
                // 清空相关缓存
                cache.delete(MyFirstLevelDisciplineController.CacheNameHelper.ListAll_CacheName);
    ...
    @ApiImplicitParam(name = "id", value = "一级学科的id", required = true, dataType = "Long", paramType = "path")
    @DeleteMapping("/firstLevelDiscipline/{id}")   //delete请求
    @ResponseBody
    public Result destroy(@PathVariable Long id){
        boolean success = myFirstLevelDisciplineService.deleteFirstLevelDiscipline(id);
        if(success){
            // 清空相关缓存
            cache.delete(MyFirstLevelDisciplineController.CacheNameHelper.ListAll_CacheName);
            return Result.build(ResultType.Success);
    ...
    public Result update(@RequestBody(required = false) FirstLevelDiscipline firstLevelDiscipline){
        if(firstLevelDiscipline != null && firstLevelDiscipline.getId() != null){
            boolean success = myFirstLevelDisciplineService.updateFirstLevelDiscipline(firstLevelDiscipline);
            if(success){
                //清除相应的缓存
                cache.delete(MyFirstLevelDisciplineController.CacheNameHelper.Receive_CacheNamePrefix + firstLevelDiscipline.getId());
                cache.delete(MyFirstLevelDisciplineController.CacheNameHelper.ListAll_CacheName);
                return Result.build(ResultType.Success);
            }else{
                return Result.build(ResultType.Failed);
