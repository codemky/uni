...
import edu.uni.userBaseInfo1.service.MySecondLevelDisciplineService;
...
@Controller
public class MySecondLevelDisciplineController {

    //把SecondLevelDiscipline的Service层接口所有的方法自动装配到该对象中
    @Autowired
    MySecondLevelDisciplineService mySecondLevelDisciplineService;
    ...
    static class CacheNameHelper{
        ...
        public static final String Receive_CacheNamePrefix = "ub1_s_secondLevelDiscipline_";
        // ub1_e_SecondLevelDisciplines_listAll
        public static final String ListAll_CacheName = "ub1_s_secondLevelDiscipline_listAll";
    }
    ...
    public void receive(@PathVariable Long id, HttpServletResponse response) throws IOException {
        ...
        //拼接缓存键名（字符串）
        String cacheName = MySecondLevelDisciplineController.CacheNameHelper.Receive_CacheNamePrefix + id;
        //尝试在缓存中通过键名获取相应的键值
        //因为在Redis中，数据是以”“” "键-值"对 的形式储存的
        String json = cache.get(cacheName);
        //如果在缓存中找不到，那就从数据库里找
        if(json == null){
            SecondLevelDiscipline secondLevelDiscipline = mySecondLevelDisciplineService.selectSecondLevelDisciplineById(id);
    ...
    public void selectAll(HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = MySecondLevelDisciplineController.CacheNameHelper.ListAll_CacheName;
        String json = cache.get(cacheName);
        if(json == null){
            json = Result.build(ResultType.Success)
                    .appendData("secondLevelDisciplines", mySecondLevelDisciplineService.selectAllSecondLevelDisciplines()).convertIntoJSON();
            cache.set(cacheName,json);
        }
        response.getWriter().write(json);
    }
    ...
    public Result create(@RequestBody(required = false) SecondLevelDiscipline SecondLevelDiscipline){
        //检验页面传来的对象是否存在
        if(SecondLevelDiscipline != null){
            boolean success = mySecondLevelDisciplineService.insertSecondLevelDiscipline(SecondLevelDiscipline);
            if(success){
                // 清空相关缓存
                cache.delete(MySecondLevelDisciplineController.CacheNameHelper.ListAll_CacheName);
    ...
    public Result destroy(@PathVariable Long id){
        boolean success = mySecondLevelDisciplineService.deleteSecondLevelDiscipline(id);
        if(success){
            // 清空相关缓存
            cache.delete(MySecondLevelDisciplineController.CacheNameHelper.ListAll_CacheName);
            return Result.build(ResultType.Success);
    ...
    public Result update(@RequestBody(required = false) SecondLevelDiscipline SecondLevelDiscipline){
        if(SecondLevelDiscipline != null && SecondLevelDiscipline.getId() != null){
            boolean success = mySecondLevelDisciplineService.updateSecondLevelDiscipline(SecondLevelDiscipline);
            if(success){
                //清除相应的缓存
                cache.delete(MySecondLevelDisciplineController.CacheNameHelper.Receive_CacheNamePrefix + SecondLevelDiscipline.getId());
                cache.delete(MySecondLevelDisciplineController.CacheNameHelper.ListAll_CacheName);
                return Result.build(ResultType.Success);
            }else{
                return Result.build(ResultType.Failed);
