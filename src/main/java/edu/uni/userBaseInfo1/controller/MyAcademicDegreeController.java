...
@Controller
public class MyAcademicDegreeController {
    ...
    public void receive(@PathVariable Long id, HttpServletResponse response) throws IOException {
        ...
        //拼接缓存键名（字符串）
        String cacheName = edu.uni.userBaseInfo1.controller.MyAcademicDegreeController.CacheNameHelper.Receive_CacheNamePrefix + id;
        //尝试在缓存中通过键名获取相应的键值
        //因为在Redis中，数据是以”“” "键-值"对 的形式储存的
        String json = cache.get(cacheName);
        //如果在缓存中找不到，那就从数据库里找
        if(json == null){
            AcademicDegree academicDegree = myAcademicDegreeService.selectById(id);
    ...
    public void selectAll(HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = MyAcademicController.CacheNameHelper.ListAll_CacheName;
        String json = cache.get(cacheName);
        if(json == null){
            json = Result.build(ResultType.Success)
                    .appendData("academicDegrees", myAcademicDegreeService.selectAll()).convertIntoJSON();
            cache.set(cacheName,json);
        }
        response.getWriter().write(json);
    }
    ...
    public Result create(@RequestBody(required = false) AcademicDegree academicDegree){
        //检验页面传来的对象是否存在
        if(academicDegree != null){
            boolean success = myAcademicDegreeService.insert(academicDegree);
            if(success){
                // 清空相关缓存
                cache.delete(edu.uni.userBaseInfo1.controller.MyAcademicDegreeController.CacheNameHelper.ListAll_CacheName);
    ...
    public Result destroy(@PathVariable Long id){
        boolean success = myAcademicDegreeService.delete(id);
        if(success){
            // 清空相关缓存
            cache.delete(edu.uni.userBaseInfo1.controller.MyAcademicDegreeController.CacheNameHelper.ListAll_CacheName);
            return Result.build(ResultType.Success);
    ...
    public Result update(@RequestBody(required = false) AcademicDegree academicDegree){
        if(academicDegree != null && academicDegree.getId() != null){
            boolean success = myAcademicDegreeService.update(academicDegree);
            if(success){
                //清除相应的缓存
                cache.delete(edu.uni.userBaseInfo1.controller.MyAcademicDegreeController.CacheNameHelper.Receive_CacheNamePrefix + academicDegree.getId());
                cache.delete(edu.uni.userBaseInfo1.controller.MyAcademicDegreeController.CacheNameHelper.ListAll_CacheName);
                return Result.build(ResultType.Success);
            }else{
                return Result.build(ResultType.Failed);
