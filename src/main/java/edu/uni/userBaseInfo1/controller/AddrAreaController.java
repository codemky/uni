...
public class AddrAreaController {
    ...
    @ApiImplicitParam(name = "id", value = "AddrArea表的一个id", required = false, dataType = "Integer" , paramType = "path")
    @ResponseBody
    public void receive(@PathVariable Integer id, HttpServletResponse response) throws IOException {
        //设置返回的数据格式
        response.setContentType("application/json;charset=utf-8");
        //拼接缓存键名（字符串）
        String cacheName = AddrAreaController.CacheNameHelper.Receive_CacheNamePrefix + id;
        //尝试在缓存中通过键名获取相应的键值
    ...
    @GetMapping("addrAreas/listAddrAreaAll")
    @ResponseBody
    public void selectAll(HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = AddrAreaController.CacheNameHelper.ListAll_CacheName;
        String json = cache.get(cacheName);
        if(json == null){
            json = Result.build(ResultType.Success)
                    .appendData("addrAreas",addrAreaService.selectAllAddrAreas()).convertIntoJSON();
            cache.set(cacheName,json);
        }
        response.getWriter().write(json);
    }
    ...
    public Result create(@RequestBody(required = false) AddrArea addrArea){
        ...
        if(addrArea != null){
            ...
            if(success){
                // 清空相关缓存
                cache.delete(AddrAreaController.CacheNameHelper.ListAll_CacheName);
    ...
    @ApiImplicitParam(name = "id", value = "城区的id", required = true, dataType = "Integer", paramType = "path")
    ...
    public Result update(@RequestBody(required = false) AddrArea addrArea){
        if(addrArea != null && addrArea.getId() != null){
            ...
            if(success){
                //清除相应的缓存
                cache.delete(AddrAreaController.CacheNameHelper.Receive_CacheNamePrefix + addrArea.getId());
                cache.delete(AddrAreaController.CacheNameHelper.ListAll_CacheName);
                return Result.build(ResultType.Success);
            }else{
                return Result.build(ResultType.Failed);
