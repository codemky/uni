...
public class StudentController {
        ...
        @ApiOperation( value = "以一个id获取一条学生信息记录详情",notes = "未测试" )
        @GetMapping("student/{id}")
        @ApiImplicitParam(name = "id", value = "Student表的一个id", required = false, dataType = "Integer" , paramType = "path")
        @ResponseBody
         ...
         * @apiNote: 查询所有学生信息记录
         */
        @ApiOperation( value = "获取所有学生信息记录的内容",notes = "未测试" )
        @GetMapping("students/listAll")
        @ResponseBody
        public void selectAll(HttpServletResponse response) throws IOException {
            response.setContentType("application/json;charset=utf-8");
            String cacheName = edu.uni.userBaseInfo1.controller.EcommController.CacheNameHelper.ListAll_CacheName;
            String json = cache.get(cacheName);
            if(json == null){
                json = Result.build(ResultType.Success)
                        .appendData("ecomms",studentService.selectAll()).convertIntoJSON();
                cache.set(cacheName,json);
            }
            response.getWriter().write(json);
        }
         ...
         * @return 删除操作结果
         */
        @ApiOperation(value="删除学生", notes="未测试")
         ...
         * @return 更新操作结果
         */
        @ApiOperation(value="更新学生信息详情", notes="未测试")
        @ApiImplicitParam(name = "student", value = "学生信息详情实体", required = true, dataType = "Student")
