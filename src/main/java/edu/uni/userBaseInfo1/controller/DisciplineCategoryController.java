...
import edu.uni.userBaseInfo1.service.DisciplineCategoryService;
...
public class DisciplineCategoryController {
    ...
    public void receive(@PathVariable Long id, HttpServletResponse response) throws IOException {
        ...
        //拼接缓存键名（字符串）
        String cacheName = DisciplineCategoryController.CacheNameHelper.Receive_CacheNamePrefix + id;
        //尝试在缓存中通过键名获取相应的键值
        //因为在Redis中，数据是以”“” "键-值"对 的形式储存的
        String json = cache.get(cacheName);
        //如果在缓存中找不到，那就从数据库里找
        if(json == null){
            DisciplineCategory disciplineCategory = disciplineCategoryService.selectById(id);
    ...
    public Result update(@RequestBody(required = false) DisciplineCategory disciplineCategory){
        if(disciplineCategory != null && disciplineCategory.getId() != null){
            ...
            if(success){
                //清除相应的缓存
                cache.delete(DisciplineCategoryController.CacheNameHelper.Receive_CacheNamePrefix + disciplineCategory.getId());
                cache.delete(DisciplineCategoryController.CacheNameHelper.ListAll_CacheName);
                return Result.build(ResultType.Success);
            }else{
                return Result.build(ResultType.Failed);
